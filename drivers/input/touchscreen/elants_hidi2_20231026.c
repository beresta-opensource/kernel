...
static void elants_i2c_mt_event_HID_I2C_Finger(struct elants_data *ts, u8 *buf)
{
    ...
    for(i = 0; i < finger_num; i++) {
		...
		if(active) {
			...
			//printk("-----elants_i2c_mt_event_HID_I2C_Finger,(%d)-----\n",__LINE__);
		}
...
static void elants_i2c_mt_event_HID_I2C_Finger_Fast(struct elants_data *ts, u8 *buf)
{
    ...
    for(i = 0; i < finger_num; i++) {
		...
		if(active) {
			...
			//printk("-----elants_i2c_mt_event_HID_I2C_Finger_Fast,(%d)-----\n",__LINE__);
...
static void elants_i2c_mt_event_HID_I2C_Pen(struct elants_data *ts, u8 *buf)
{
	...
	//dynamically set length from 2nd pen packet
	recv_size = (buf[3] == 0x00 ? PACKET_SIZE : buf[0]);
	dev_dbg(&ts->client->dev, "recv_size = %d\n", recv_size);
	...
	//printk("-----elants_i2c_mt_event_HID_I2C_Pen,(%d)-----\n",__LINE__);
	input_sync(idev);
}
...
static void  elan_ts_work_func(struct work_struct *work)
{
	...
	len = i2c_master_recv(client, ts->buf, recv_size/*sizeof(ts->buf)*/);
	if (len < 0) {
