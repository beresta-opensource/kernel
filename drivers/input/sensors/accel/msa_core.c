...
//#define MTK_ANDROID_M

static short offsetz = 0;
...
#define MSA_NSA_INIT_SECTION                            { NSA_REG_G_RANGE,              0xc0,   0x03    },                                  \
                                                        ...
                                                        { NSA_REG_INT_PIN_CONFIG,    	  0x00,   0x05    },                                  \
                                                        { -1,     						0x00,   0x00    },                                  \
                                                        { -1,     						0x00,   0x00    },                                  \
                                                        { -1,                 0x00,   0x00    },                                  \
                                                        { -1,                           0x00,   0x00    },                                  \
...
#if !defined(MTK_ANDROID_M) || !MTK_ANDROID_M
#define abs_msa_core(x) ({ long __x = (x); (__x < 0) ? -__x : __x; })
#endif 
...
static char isXStick(void){
	...
	for (i = 0; i < AIX_HISTORY_SIZE; i++){
	    if ((abs_msa_core(aixHistort[i*3]) < STICK_LSB)&&(aixHistort[i*3] != 0)){
	        break;
	    }
	}
...
static char isYStick(void){
	int i=0,j=0,temp=0;
	for (i = 0; i < AIX_HISTORY_SIZE; i++){
	    if ((abs_msa_core(aixHistort[i*3+1]) < STICK_LSB)&&(aixHistort[i*3+1] != 0)){
	        break;
	    }
	}
...
static char isZStick(void){
	int i=0,j=0,temp=0;
	for (i = 0; i < AIX_HISTORY_SIZE; i++){
	    if ((abs_msa_core(aixHistort[i*3+2]) < STICK_LSB)&&(aixHistort[i*3+2] != 0)){
	        break;
	    }
	}
...
static short filter_average(short preAve, short sample, int paramN, int* refNum)
{
    ...
    if( abs_msa_core(sample-preAve) > PEAK_LVL  && *refNum < 3  ){ 
...
static int filter_average_enhance(FilterAverageContext* fac, short sample)
{
    ...
    if (fac->filter_param_l == fac->filter_param_h){
        ...
        if (abs_msa_core(fac->sample_l- fac->sample_h) > fac->filter_threhold){
...
{
    unsigned char    tmp_data[6] = {0},tempdata;
    short temp = 0;
    int   res = 0;
    ...
    *z = ((short)(tmp_data[5] << msa_gsensor_drv.obj[gsensor_mod].data.data_fmt.msbw | tmp_data[4]))>> (8-msa_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
	...
	res = msa_register_read(handle, 0x14, &tempdata);
    if(tempdata == 0x20)
     {
        temp = *x;
        *x = *y;
...
static int NSA_once_calibrate(MSA_HANDLE handle, int coarse_step[3], int fine_step[3], int xyz[3])
{
    ...
    fine[0] =  offset_data[3];
    fine[1] =  offset_data[4];
    fine[2] =  offset_data[5];
    
    MI_MSG("Old coarse_x = %d; coarse_y = %d; coarse_z = %d; fine_x = %d; fine_y = %d; fine_z = %d;", coarse[0], coarse[1], coarse[2], fine[0], fine[1], fine[2]);
        
    target[0] = xyz[0];
 		target[1] = xyz[1];
		target[2] = squareRoot(1024*1024 - target[0]*target[0] - target[1]*target[1]);
		target[2] = (xyz[2] > 0) ? target[2] : (-target[2]);

    offsetz = target[2] - xyz[2];
...
static int NSA_calibrate(MSA_HANDLE handle, int coarse_step[3], int fine_step[3], int fine_max, int target[3])
{
    ...
    if( (abs_msa_core(target[0]) + abs_msa_core(target[1]) + abs_msa_core(target[2])) != 0 && (abs_msa_core(target[0]) + abs_msa_core(target[1]) + abs_msa_core(target[2])) != 1024 ) {
        MI_ERR("Invalid argument !");
        return -1;
    }
        
    /* 0 means auto detect z direction assume z axis is verticle */
    if ((abs_msa_core(target[0]) + abs_msa_core(target[1]) + abs_msa_core(target[2])) == 0){
...
EXIT_16G_MOD:
    ...
    for (i = 0; i < nLoop; i++){
        ...
        for(j = 0; j < 3; j++){
            ...
            if( abs_msa_core(xyz[j]-target[j]) < MSA_OFFSET_THRESHOLD ){
                fine_ok_map |= (1<<j);
...
static int NSA_NTO_cali_step_calc(MSA_HANDLE handle, int coarse[3], int x100_fine[3], int x100_cust[3])
{
    ...
    for(i = 0;i < 3;i++) {        
        // *100*1000
        total_gain[i] = ((1000 + (fine_gain[i]&0x1F)*1000/32)/15)  * coarse_gain_map[(fine_gain[i]>>5)&0x07];
        coarse[i] = (int)(total_gain[i] * 500 / 100000);
        x100_fine[i] =  ((1000 + (fine_gain[i]&0x1F)*1000/32)/15) * 589 / 100;
...
#define AUTO_CALI_THRESHOLD_Z        800
...
static int msa_auto_cali_condition_confirm(int x, int y, int z, int ave_xyz[3])
{
    ...
    stable_sample_sum[2] = 0;
        ...
        x_ok =  (abs_msa_core(ave_xyz[0]) < AUTO_CALI_THRESHOLD_XY) ? 1:0;
        y_ok =  (abs_msa_core(ave_xyz[1]) < AUTO_CALI_THRESHOLD_XY) ? 1:0;
        z_ok =  (abs_msa_core(abs_msa_core(ave_xyz[2])-1024) < AUTO_CALI_THRESHOLD_Z) ? 1:0;

        if( (abs_msa_core(max-min) > STABLE_CHECK_THRESHOLD) ||((x_ok + y_ok + z_ok) < 3) ) {
            return -1;
        }
