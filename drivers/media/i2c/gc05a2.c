...
//  i2c  adders  0x6e  or 0x7e

#define MEDIA_BUS_FMT_SRGGB10_GC MEDIA_BUS_FMT_SGRBG10_1X10
#define GC05A2_REG_VALUE_08BIT		1
#define GC05A2_REG_VALUE_16BIT		2


#define GC05a2_LANES			2
#define GC05a2_BITS_PER_SAMPLE		10
#define GC05a2_LINK_FREQ_MHZ		451000000LL
#define MIPI_FREQ		451000000LL

/* pixel rate = link frequency * 2 * lanes / BITS_PER_SAMPLE */
#define GC05a2_PIXEL_RATE		(MIPI_FREQ * 2LL * 2LL / 10)
#define GC05a2_XVCLK_FREQ		24000000

#define CHIP_ID				0x00a200
#define GC05a2_REG_CHIP_ID_H		0x3f0
#define GC05a2_REG_CHIP_ID_L		0x3f1
...
#define GC05a2_AGAIN_H		0x0204
#define GC05a2_AGAIN_l		0x0205

#define GC05a2_GAIN_MIN			1024
#define GC05a2_GAIN_MAX			1024*16
#define GC05a2_GAIN_STEP		1
#define GC05a2_GAIN_DEFAULT		1024
...
#define DD_PARAM_QTY_05a2	200
#define INFO_ROM_START_05a2	0x08
#define INFO_WIDTH_05a2		0x08
#define WB_ROM_START_05a2	0x88
#define WB_WIDTH_05a2		0x05
#define GOLDEN_ROM_START_05a2	0xe0
#define GOLDEN_WIDTH_05a2	0x05
...
struct gc05a2 {
	...
	const char		*len_name;
	u32 Dgain_ratio;
	struct gc05a2_otp_info *otp;
	struct rkmodule_inf	module_inf;
	struct rkmodule_awb_cfg	awb_cfg;
};

#define to_gc05a2(sd) container_of(sd, struct gc05a2, subdev)
extern int rk817_ldo7_disable(void);
extern int rk817_ldo7_enable(void);
extern int rk817_ldo8_disable(void);
extern int rk817_ldo8_enable(void);
extern int rk817_ldo9_disable(void);
extern int rk817_ldo9_enable(void);
...
static int gc05a2_write_reg(struct i2c_client *client, u16 reg,
			    u32 len, u32 val)
{
	u32 buf_i, val_i;
	u8 buf[6];
				...
				const struct regval *regs)
{
	...
	for (i = 0; ret == 0 && regs[i].addr != REG_NULL; i++)
		ret = gc05a2_write_reg(client, regs[i].addr, 
				       GC05A2_REG_VALUE_08BIT, regs[i].val);

	return ret;
}

/* Read registers up to 4 at a time */
static int gc05a2_read_reg(struct i2c_client *client, u16 reg,
	...
	struct v4l2_subdev_format *fmt)
{
	...
	fmt->format.code = MEDIA_BUS_FMT_SRGGB10_GC;
	...
	struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->index != 0)
		return -EINVAL;
	code->code = MEDIA_BUS_FMT_SRGGB10_GC;
				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	if (fse->code != MEDIA_BUS_FMT_SRGGB10_GC)
		return -EINVAL;
...
static int __gc05a2_start_stream(struct gc05a2 *gc05a2)
{
	...
	ret |= gc05a2_write_reg(gc05a2->client,
		GC05a2_REG_CTRL_MODE,GC05A2_REG_VALUE_08BIT,
...
static int gc05a2_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		printk("gc05a2_s_power yyk22  %d\n", gpiod_get_value_cansleep(gc05a2->pwdn_gpio));
...
static int __gc05a2_power_on(struct gc05a2 *gc05a2)
{
	...
	printk("__gc05a2_power_on %d yyk", delay_us);
	return 0;
...
static void __gc05a2_power_off(struct gc05a2 *gc05a2)
{
	...
	printk("__gc05a2_power_off elvis");
...
static int gc05a2_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	try_fmt->code = MEDIA_BUS_FMT_SRGGB10_GC;
				       ...
				       struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (fie->code != MEDIA_BUS_FMT_SRGGB10_GC)
		return -EINVAL;
...
static int gc05a2_set_test_pattern(struct gc05a2 *gc05a2, int value)
{
	...
	//ret = gc05a2_write_reg(gc05a2->client, 0x008d, value);
	return ret;
}
...
static int gc05a2_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		max = gc05a2->cur_mode->height + ctrl->val - 4;
	...
	case V4L2_CID_EXPOSURE:
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_REG_EXPOSURE_H,GC05A2_REG_VALUE_08BIT,
			(ctrl->val&0xFF00) >> 8);
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_REG_EXPOSURE_L,GC05A2_REG_VALUE_08BIT,
			(ctrl->val&0x00FF));	
		break;
	case V4L2_CID_ANALOGUE_GAIN:		
	/* 6.10 */
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_AGAIN_H,GC05A2_REG_VALUE_08BIT,
			(ctrl->val&0xFF00) >> 8);
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_AGAIN_l,GC05A2_REG_VALUE_08BIT,
			(ctrl->val&0x00FF));
		break;
	case V4L2_CID_VBLANK:
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_REG_VTS_H,GC05A2_REG_VALUE_08BIT,
			((ctrl->val + gc05a2->cur_mode->height) >> 8) & 0xff);
		ret |= gc05a2_write_reg(gc05a2->client,
			GC05a2_REG_VTS_L,GC05A2_REG_VALUE_08BIT,
			(ctrl->val + gc05a2->cur_mode->height) & 0xff);
		break;
	...
	struct i2c_client *client)
{
	...
	ret = gc05a2_read_reg(client, GC05a2_REG_CHIP_ID_H,
			      GC05A2_REG_VALUE_16BIT, &reg_H);

	ret |= gc05a2_read_reg(client, GC05a2_REG_CHIP_ID_L,
			      GC05A2_REG_VALUE_16BIT, &reg_L);
